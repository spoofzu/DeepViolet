...
/**
 * DeepViolet X.509 certificate representation. Most values come from an
 * java.security.cert.X509Certificate instance upon initialization. However,
 * some values are computed for convenience like CERT_FINGERPRINT which represents
 * the certificate fingerprint computed with the signing algorithm.
 * 
 * @author Milton Smith
 */
public interface IX509Certificate {
	 ...
	 * Critical OIDs.
	 *
	 * @return Property names of critical OIDs.
	 */
	 ...
	 * @param key
	 *            Key name of key/value pair.
	 * @return Property values for critical OIDs
	 */
	 ...
	 *             Thrown on problems.
	 */
	IX509Certificate[] getCertificateChain() throws DeepVioletException;

	/**
	 * Public key algorithm (RSA, EC, DSA, etc.)
	...
	/**
	 * For EC keys, the curve name (e.g., secp256r1). Null for non-EC keys.
	 * @return Curve name or null
	 */
	String getPublicKeyCurve();
	 ...
	 * @return List of SANs (DNS names, IPs, emails, URIs). Empty if no SAN extension.
	 */
	List<String> getSubjectAlternativeNames();

	/**
	 * Get revocation status for this certificate.
	 * @return Revocation status or null if not checked.
	 */
	 ...
	 * an instance of {@code IX509Certificate}, 2) all key/value pairs are
