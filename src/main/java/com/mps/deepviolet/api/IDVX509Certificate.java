...
/**
 * DeepViolet X.509 certificate represenation.  Most values come
 * from an java.security.cert.X509Certificate instance upon
 * initialization.  However, some values are computed for
 * convience like CERT_FINGERPRINT which represents the certificate
 * fingerprint computed with the signing alorithm.
 * @author Milton Smith
 */
public interface IDVX509Certificate {
	 ...
	 * identifies the subject.  Usually the subject is the host but could be other things
	 ...
	 * the default trust store and root Certificate Authorities that ship
	 * with Java.  Note: roots change between different versions
	 * of Java.  Oracle removes, adds, and updates roots regularly.
	 * @return true, the current certificate is a root in the active
	 ...
	 * {@code IDVX509Certificate}, 2) all key/value pairs are equal, 3) the length of
