...
public class DVFactory {
	 ...
	 */
	public static final synchronized IDVSession initializeSession(URL url)
			throws DVException {
		...
		try {
			...
			socket = (SSLSocket) factory.createSocket(url.getHost(),
					url.getDefaultPort());
			...
			for (InetAddress address : addresses) {
				...
				ImmutableDVHost dvhost = new ImmutableDVHost(host, ip,
						cannonical, url);
				list.add(dvhost);
			}
			...
			String client_auth_req = Boolean.toString(socket
					.getNeedClientAuth());
			String client_auth_want = Boolean.toString(socket
					.getWantClientAuth());
			...
			session = new MutableDVSession(url,
					(IDVHost[]) list.toArray(new ImmutableDVHost[0]));
