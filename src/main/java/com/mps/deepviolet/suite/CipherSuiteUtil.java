...
/**
 * Utility class to handle cryptographic functions. Significant contributions
 * around ciphersuite handling adapted from code examples by Thomas Pornin
 * <pornin@bolet.org>.
 * 
 * @author Milton Smith
...
public class CipherSuiteUtil {
	...
	private static final Logger logger = LoggerFactory
			.getLogger("com.mps.deepviolet.suite.CipherSuiteUtil");
	...
	private static final SSLSocketFactory dsc = HttpsURLConnection
			.getDefaultSSLSocketFactory();

	private static final HostnameVerifier dhv = HttpsURLConnection
	...
	// todo this needs to go bye bye. bad idea. I'm thinking a better way to do
	// this is eventually
	// get to a MVC type architecture. This would better address the ways
	// deepviolet can be used
	public static synchronized ServerMetadata getServerMetadataInstance(URL url)
			throws Exception {

		HostData hostdata = null;
		Boolean compress = false;

		// return cached instance of server TLS data if available and not
		// expired.
		...
		for (int v = 0x0300; v <= 0x0303; v++) {
			...
			if (sh.compression == 1) {
				compress = true;
				hostdata.setScalarValue("getServerMetadataInstance",
		...
		if (sh2 != null) {
			...
			for (int c : vc2) {
				if (!vulnFREAK) {
					vulnFREAK = cipherSuiteStringV2(c).indexOf("EXPORT") > -1;
				}
				listv2.add(cipherSuiteStringV2(c) + "(0x"
			...
			if (sh2.serverCertName != null) {
				hostdata.setScalarValue("getServerMetadataInstance",
						sh2.serverCertHash, sh2.serverCertName);
			}
			hostdata.setVectorValue("getServerMetadataInstance",
					versionString(0x0200), listv2.toArray(tmp));
		...
		for (int v : sv) {
			...
			if (lastSuppCS == null || !lastSuppCS.equals(vsc)) {
				...
				for (int c : vsc) {
					if (!vulnFREAK) {
						vulnFREAK = cipherSuiteString(c).indexOf("EXPORT") > -1;
					}
					listv.add(cipherSuiteString(c) + "(0x"
							+ Integer.toHexString(c) + ")");
				}
				lastSuppCS = vsc;
			...
			hostdata.setVectorValue("getServerMetadataInstance",
					versionString(v), listv.toArray(tmp));
		...
		// hostdata.setScalarValue("analysis","MINIMAL_ENCRYPTION_STRENGTH",
		// strengthString(agMinStrength));
		// hostdata.setScalarValue("analysis","ACHIEVABLE_ENCRYPTION_STRENGTH",
		// strengthString(agMinStrength));
		// hostdata.setScalarValue("analysis","BEAST_VULNERABLE", vulnBEAST ?
		// "vulnerable" : "protected");
		// hostdata.setScalarValue("analysis","CRIME_VULNERABLE", compress ?
		// "vulnerable" : "protected");
		// hostdata.setScalarValue("analysis","FREAK_VULNERABLE", vulnFREAK ?
		// "vulnerable" : "protected");
	 ...
	 * <code>javax.security.cert.X509Certificate</code> to
	 * <code>java.security.cert.X509Certificate</code>. No doubt, a bit of
	 * legacy.
	 * 
	 * @param cert
	 *            X509 certificate to convert.
	 * @return java.security.cert.X509Certificate Converted object
	 * @see http 
	 *      ://exampledepot.8waytrips.com/egs/javax.security.cert/ConvertCert
	 *      .html
	 */
	 ...
	 * 
	 * @param protocol
	 *            Ciphersuite protocol to test.
	 * @return String indicating strength, CLEAR(no encryption), WEAK, MEDIUM,
	 *         STRONG.
	 */
	public static final String getStrength(String protocol) {
		...
		} else if (protocol.contains("DES40") || protocol.contains("_40_")
				|| protocol.contains("_EXPORT40_")) {

			result = weak;

		} else if (protocol.contains("_DES_") || protocol.contains("_DES64_")
	 ...
	 *             Thrown on problems.
	 */
	public static final X509Certificate getServerCertificate(URL url)
			throws Exception {
	 ...
	 *             Thrown on problems.
	 */
	public static final Map<String, List<String>> getHttpResponseHeaders(URL url)
			throws Exception {
	...
	// public static final synchronized ServerMetadata
	// getHttpResponseHeaders(URL url) throws Exception {
	//
	// HostData hostdata = null;
	//
	// // return cached instance of server TLS data if available and not
	 ...
	 * 
	 * @param value
	 *            true, chain will be tested. false, chain will not be tested.
	 * @throws Exception
	 */
	public static final void enableTLSChainTesting(boolean value)
			throws Exception {
	 ...
	 *             on problems.
	 * @see <a
	 *      href="http://stackoverflow.com/questions/19723415/java-overriding-function-to-disable-ssl-certificate-check">java-overriding-function-to-disable-ssl-certificate-check</a>
	 */
	public static final X509Certificate[] getServerCertificateChain(URL url)
			throws Exception {
		...
		try {
			...
			for (Certificate cert : certs) {
				...
				if (cert instanceof X509Certificate) {
					...
					logger.info("Unsupported certificate type.  type="
	 ...
	 * @throws Exception
	 * @see http 
	 *      ://stackoverflow.com/questions/3508050/how-can-i-get-a-list-of-trusted
	 *      -root-certificates-in-java
	 */
	public static final X509Certificate[] getJavaRootCertificates()
			throws Exception {

		// TODO: Maybe be good to consider caching this at some point (at least
		// for a few seconds)
	...
	// /**
	// * Test to see if a particular SHA1 hash is a root in the Java system
	// keystore.
	// * @param sha1hash
	// * @return true, SHA1 hash belongs to a Java root. false, no Java root
	// found.
	// */
	// public static final boolean isJavaRootCertificateSHA1(String sha1hash)
	// throws Exception {
	//
	...
	/**
	 * Test to see if a particular IssuerDN is a root in the Java system
	 * keystore.
	 * 
	 * @param IssuerDN
	 * @return true, IssuerDN matches a Java root. false, no matching IssuerDN
	 *         found.
	 */
	public static final boolean isJavaRootCertificateDN(String IssuerDN)
			throws Exception {
	 ...
	 * 
	 * @param certs
	 *            Chain of X509Certificates to test.
	 * @param url
	 ...
	 * @throws IOException
	 * @return True, the certificate chain is trusted. False, the chain is not
	 *         trusted.
	 */
	public static final boolean checkTrustedCertificate(
			X509Certificate[] certs, URL url) throws KeyStoreException,
		...
		// you could use a non-default KeyStore as your truststore too, instead
		// of null.
	...
	// check OCSP
	// public static final String checkOCSPStatus(X509Certificate cert,
	 ...
	 * 
	 * @param cert
	 *            Target certificate to test.
	 * @return boolean True, certificate is self-signed. False, certificate is
	 *         not self-signed.
	 */
	 ...
	 * @param String
	 *            signatureAlgorithm algorithm the certificate was signed, ex:
	 *            SHA256
	 * @return String Signer fingerprint in hex.
	 * @throws NoSuchAlgorithmException
	 */
	public static final String signerFingerprint(byte[] der,
			String signatureAlgorithm) throws NoSuchAlgorithmException {
	...
	// * @throws NoSuchAlgorithmException
	// */
	// public static final String sha1Fingerprint( byte[] der ) throws
	...
	// * @throws NoSuchAlgorithmException
	// */
	// public static final String md5Fingerprint( byte[] der ) throws
	// NoSuchAlgorithmException {
	//
	 ...
	 * 
	 * @param oidkey
	 *            OID number sequence. Ex: 2.5.29.15
	 * @return Human readable String representation of the OID number sequence.
	 *         Ex: keyusage
	 */
	 ...
	 * 
	 * @param data
	 *            byte[] of <code>der</code> encoded data
	 * @return <code>ASN1Primitive</code> representation of <code>der</code>
	 *         encoded data
	 * @throws IOException
	 * @see http 
	 *      ://stackoverflow.com/questions/2409618/how-do-i-decode-a-der-encoded
	 *      -string-in-java
	 */
	public static final ASN1Primitive toDERObject(byte[] data)
			throws IOException {
	...
	/**
	 * Reentrant method to decode ASN1Primiatives. ASN1Primiatives types
	 * handled: <code>DEROctetString</code>, <code>DLSequence</code>,
	 * <code>DERSequence</code>, <code>DERIA5String</code>,
	 * <code>DERBitString</code>, <code>ASN1Boolean</code>,
	 * <code>ASN1Integer</code>, <code>DERTaggedObject</code>,
	 * <code>ASN1ObjectIdentifier</code>. NOTE: this does not decode all OIDs.
	 * For supported OIDs please refer to the code. This is mostly a trial and
	 * error process as follows, 1) review logs for unsupported OIDs, 2) include
	 * code to support new OIDs, 3) explore more web sites, 4) goto step 1
	 * 
	 * @param primitive
	 * @param buff
	 * @throws IOException
	 */
	public static final void walkASN1Sequence(ASN1Primitive primitive,
		...
		if (primitive instanceof DEROctetString) {
			...
			// TODO: May be useful to parse differently depending upon tag type
			// in future.
			...
			// Assistance by
			// https://svn.cesecore.eu/svn/ejbca/branches/Branch_3_11/ejbca/conf/extendedkeyusage.properties
			...
			// Assistance fm
			// http://stackoverflow.com/questions/16058889/java-bouncy-castle-ocsp-url
			...
			if (t.getTagNo() == 6) { // Several
				...
				logger.info("Unhandled DERTaggedObject type. RAW="
						+ buff2.toString());
			}
			...
			logger.error("Unhandled primitive data type, type="
	 ...
	 *      -string-in-java
	 */
	public static final String getExtensionValue(
			X509Certificate X509Certificate, String oid) throws IOException {
	...
	static boolean testBEAST(InetSocketAddress isa, int version,
			Set<Integer> supp) {
		/*
